...
import json
import numpy as np
def loadavgtimes(replicatype,activeType,totaltime,period):
    filename='./LRUtestData-'+str(replicatype)+'-'+str(activeType)+'-'+str(totaltime)+'-'+str(period)+'-2016-200-200-10-1-4-'+str(period)+'-1.txt'
    AvgTime=[]
...
def plotAvg():
    ...
    step = 1
    beginblock = 200
    Avgtime01, b01, e01, s01 = loadavgtimes(replicatype, activeType, totaltime,0)
    Avgtime02, b02, e02, s02 = loadavgtimes(replicatype, activeType, totaltime,1)
    Avgtime12, b12, e12, s12 = loadavgtimes(replicatype, activeType, totaltime,2)
    Avgtime22, b22, e22, s22 = loadavgtimes(replicatype, activeType, totaltime,3)
    ...
    plt.plot(range(b01, e01, s01), Avgtime01, color=colors[0], label='0')
    plt.plot(range(b01, e01, s01), Avgtime02, color=colors[1], label='1')
    plt.plot(range(b01, e01, s01), Avgtime12, color=colors[2], label='2')
    plt.plot(range(b01, e01, s01), Avgtime22, color=colors[3], label='3')
