...
# 初始散布中散布的副本数目，以及在驱逐的时候最低应该保留的副本数目
LEFT_BLOCKS=3

# 某一batch中，第一个编码分片的编号等于它所在batch中首个数据区块的编号加上ENC_BLK_IDX
# 第二个编码分片的编号等于他所在batch中第一个数据区块的编号加上ENC_BLK_IDX+1
# 以此类推
ENC_BLK_IDX=10000
...
#记录所有已经被编码的区块的编号，对这些区块单独处理其驱逐过程
# encoded_blks={0}

# 记录所有已经被编码的区块的集合，例 [[65550,65551],[65559,65554]]
# 其位置与encpde_shard_pos对应
encode_blks_set=[]

# 存储编码区块的位置，每一个编码区块只能被一个节点存储，其中id与encode_blks_set中的位置对应，存储两个节点编号，分别是存储了第一个编码区块和第二个编码区块的节点
# 用于我们的方案
# 例：[[0,1,avgsize],[2,7,avgsize]]
encode_shard_pos=[]

# 存储编码分片的位置，每一个编码区块可能被多个节点存储，编码分片的id就是其对应首个分片的id加上10000、10001，以此类推。因此，不允许编码分片个数超过其数据分片的个数。
# 用于bft
# 例：{664001:[0,1,avgsize],66402:[9,2,avgsize]}
encode_shard_pos_bft={}
...
def static_assign_blocks(piece,period,type):
    ...
    其中的参数含义如下：
    `piece`: 指定静态散布散布几个副本
    `period`: 指定静态散布副本的生命周期
    `type`: 指定静态散布策略，可选'piece'与'piecekad'：
        - `piece`：随机选择piece个不同节点作为副本存储位置
        - `piecekad`：选择距离当前数据id最近的piece个节点作为副本存储位置
...
def assign_one_block(blockID,piece,period,type):#,popularity_epoch_list):
    ...
    # if len(master_node)>1000:
    #     print(len(master_node),':',blockID,piece,period,type)
...
def bftstore(piece=3,databatch=8,encodebatch=2):
    """(int,int,int)

    BFT-STORE的散布。

    其中的参数含义如下：
    `piece`：每一个数据块和编码块存储的副本个数
    `databatch`：几个区块进行一次编码，每一个区块形成一个数据块
    `encodebatch`：编码块的个数
    ...
    global block_last_access_epochid
    global LEFT_BLOCKS
    global blocks_in_which_nodes_and_timelived
    LEFT_BLOCKS=piece
    nodes_storage_used=[0]*nodes_num
    BLOCK_ID=dsinit.getBlockID()
    #init nodes_stored_blocks_popularity
    ...
    block_sum=len(BLOCK_ID)
    if encodebatch>databatch:
        print('错误：运行bftstore时，不允许数据分片个数小于编码分片个数！')
    for i in range(math.ceil(block_sum/databatch)):
        #处理所有数据块
        totalsize=0
        blockIDs=[]
        startBlockID=beginID+databatch*i
        startEncodeIDEVE=startBlockID+ENC_BLK_IDX
        # storeplacedebug={}
        for bid in range(startBlockID,startBlockID+databatch):
            if bid<endID:
                blockIDs.append(bid)
                totalsize+=blocksizes[bid-beginID]
                # storeplacedebug[bid]=[]
        bsize_ac=len(blockIDs)
        avgsize=totalsize/bsize_ac
        for bid in range(startEncodeIDEVE,startEncodeIDEVE+encodebatch):
            blockIDs.append(bid)
        # 不计算bfttsore的编码块占据空间，所以统计可能不准确
        for nodeID in range(nodes_num):
            for times in range(piece):
                place=(nodeID+nodes_num-times)%nodes_num
                #如果算出来的是数据分片，当作区块存储
                if place<bsize_ac:
                    # print(blockIDs[place])
                    store_block_to_node(blockIDs[place],nodeID,np.inf)
                # 如果算出来的是编码分片，当作编码存储
                elif place<len(blockIDs):
                    if blockIDs[place] not in encode_shard_pos_bft:
                        encode_shard_pos_bft[blockIDs[place]]=[nodeID]
                    else:
                        encode_shard_pos_bft[blockIDs[place]].append(nodeID)
                    # storeplacedebug[blockIDs[place]].append(nodeID)
                    # blocks_in_which_nodes_and_timelived[blockIDs[place]-beginID][nodeID]=np.inf
                # store_block_to_node(blockIDs[(nodeID+nodes_num-1)%nodes_num],nodeID,np.inf)
                # store_block_to_node(blockIDs[(nodeID+nodes_num-2)%nodes_num],nodeID,np.inf)
        # 处理encode_shard_pos_bft中的avgsize
        for bid in range(startEncodeIDEVE,startEncodeIDEVE+encodebatch):
            encode_shard_pos_bft[bid].append(avgsize)
        
        # if i==0:
        #     print(storeplacedebug)
    # print('bft'+str(len(blocks_in_which_nodes_and_timelived[0])))
def passive_dynamic_replication_one_node(chosen_blocks,nodeID,passive_type,sort_value_dict,period,lambdai):#,popularity_passing_dict):
    ...
    if len(passive_type)<3 or passive_type[len(passive_type)-3:]!='kad':
        ...
        # if len(all_blocks_replicated)!=0:
        #     print('total=',len(all_blocks_replicated))
    ...
    if active_type=='random':
        ...
        for blocks_i in blocks_to_be_offload:
            ...
            for thres_i in range(len(thres)):
                # print(nodes_stored_blocks_popularity[nodeID][blocks_i][1],thres[thres_i])
                if nodes_stored_blocks_popularity[nodeID][blocks_i][1]>=thres[thres_i]:
                    ...
                    # storedtotal+=replica_nums_thisnode
            ...
            for store_blk_nodes in store_nodes_tmp:
                ...
                stored_blocks[store_blk_nodes[0]]=blocks_i# 记录往store_blk_nodes[0]这个节点存储了区块blocks_i
        # print(len(stored_blocks),len(blocks_to_be_offload))
        # if storedtotal!=0:
        #     print('total=',storedtotal)
        return [],len(blocks_to_be_offload),stored_blocks
    else:
...
def store_block_to_node(blockID,nodeID,period):#,popularity_value):
    ...
    # print('store:'+str(len(blocks_in_which_nodes_and_timelived)))
    if nodeID not in blocks_in_which_nodes_and_timelived[blockID-beginID]:
    ...
    # print('sb+'+str(len(blocks_in_which_nodes_and_timelived[blockID-beginID])))
    # update popularity
...
def update_livetime_and_expel(end_since,epochs,fp):
    ...
    for blockID in range(beginID,end_since):
        for nodeID in dead_blocks[blockID-beginID]:
            ...
            elif len(blocks_in_which_nodes_and_timelived[blockID-beginID])==2+1 and (True in [blockID in eb for eb in encode_blks_set]):
                # expeltotal+=1
                nodes_stored_blocks_popularity[nodeID].pop(blockID)
                nodes_storage_used[nodeID]-=blocksizes[blockID-beginID]
                # update time_lived
    ...
    # nothing to return
    # if(expeltotal!=0):
    #     print('total=',expeltotal)
    return delete_blocks_debug

# @decaperated
def scan_for_encode_past(end_since,encode_epoch_limit,batch_size,encode_piece_count,beginID,time_to_expel):
    #debug info
    # node_delete=0
    # node_add=0
    # storage_delete=0
    ...
    for nodeid in range(nodes_num):
        ...
        if len(kvs)>=batch_size and kvs[batch_size-1][1]<=end_since-encode_epoch_limit:
            ...
            for blockID in encode_block_id:
                # print(blockID)
                # print(len(blocks_in_which_nodes_and_timelived))
            ...
            if len(involved_nodes)==batch_size:
                ...
                corresponding_nodeid=np.random.choice(list(involved_nodes),size=batch_size,replace=False).tolist()
            encode_nodeid=[]
            for i in range(nodes_num):
    ...
    for nodeid in range(nodes_num):
        ...
        if len(kvs)>=batch_size and kvs[batch_size-1][1]<=end_since-encode_epoch_limit:
            ...
            for blockID in encode_block_id:
                ...
                for stored_id in blocks_in_which_nodes_and_timelived[blockID-beginID].keys():
                    involved_nodes.add(stored_id)
            # involved_nodes 中存储不包含主节点的所有宿主节点
            involved_nodes.discard(nodeid)
            # 本方案的编码块固定为两个，副本数固定为2个，因此只需要主节点删除自己存的区块，
            # 随后寻找另一个不包含在involved_nodes中的节点存储另一个编码块即可
            # get data piece set and encode piece set
            # corresponding_nodeid=[]
            # if len(involved_nodes)==batch_size:
            #     corresponding_nodeid=list(involved_nodes)
            ...
            encode_nodeid=[]
            for i in range(nodes_num):
                if not i in involved_nodes and i!=nodeid:
                    encode_nodeid.append(i)
            if len(encode_nodeid)<1:
                random_node=np.random.choice(list(involved_nodes))
                encode_nodeid.append(random_node)
            # assign and update
            blocksize_sum=0
            # print('storage =',sum(nodes_storage_used),'nodeid=',nodeid)
            # 将每一个选出的区块从他的主节点删除
            encode_blks_set.append(set(encode_block_id))
            for block_id in range(len(encode_block_id)):
                blockID=encode_block_id[block_id]
                # marked as nencodeed
                block_last_access_epochid[master_node[blockID-beginID]].pop(blockID)
                # block_last_access_epochid[master_node[blockID-beginID]][blockID]=np.inf
                # for nodeID in blocks_in_which_nodes_and_timelived[blockID-beginID]:
                # update popularity
                nodes_stored_blocks_popularity[nodeid].pop(blockID)
                ...
                ##
                blocksize_sum+=blocksizes[blockID-beginID]
            # print('total delete size in nodeid=',nodeid,' is ',blocksize_sum)
            # print('length of encode blks is ',len(encode_block_id),', batch size is ',batch_size)
            blocksize_avg=blocksize_sum/batch_size
            # encode piece assign
            # 把两个编码块放在主节点和另一个被选中的节点
            # for i in range(encode_piece_count):
            this_node=encode_nodeid[random.randint(0,len(encode_nodeid)-1)]
            nodes_storage_used[this_node]+=blocksize_avg
            nodes_storage_used[nodeid]+=blocksize_avg
            # 存储这一批区块的编码分片的位置
            encode_shard_pos.append([this_node,nodeid,blocksize_avg])
            # print('total storage size add in encode piece is ',2*blocksize_avg,', master node is ',nodeid)
            # print('storage=',sum(nodes_storage_used),'nodeid=',nodeid)
                ##
                # storage_add+=blocksize_avg
                ##
...
GET_TYPE='piecekad2'#意为非bft下，两个编码块，还有 noencode和bft2


def getminvec(nodeID,blockID):
    global blocks_in_which_nodes_and_timelived
    BLOCK_ID=dsinit.getBlockID()
    # print('all'+str(len(blocks_in_which_nodes_and_timelived)))
    # print('||len"'+str(blockID-beginID))
    # print('single len'+str(len(blocks_in_which_nodes_and_timelived[blockID-beginID])))
    # print('gxor'+str(len(blocks_in_which_nodes_and_timelived[0])))
    storage_nodes_of_blockID=blocks_in_which_nodes_and_timelived[blockID-beginID].keys()
    if(nodeID in blocks_in_which_nodes_and_timelived[blockID-beginID]):
        return [[-1,0]]
...
def get_block_from_which_xor(nodeID,blockID):
    ...
    '''
    global GET_TYPE
    min_vector=getminvec(nodeID,blockID)
    if min_vector==[[-1,0]]:
        return [[-1,0],[],[],[]]
    # 同时返回两个编码块的位置以及开销
    # 
    ...
    return_encode2=[]
    return_other_blks=[]
    if GET_TYPE=='piecekad4-2':
        res_in=[blockID in ebs for ebs in encode_blks_set]
        if True in res_in:
            # 找到在第几个
            pos=res_in.index(True)
            encode_pos=encode_shard_pos[pos]
            time_cost=[communication_cost_ori[nodeID][encode_pos[posi]]*encode_pos[2] for posi in range(2)]
            return_encode1=[[encode_pos[0],time_cost[0]]]
            return_encode2=[[encode_pos[1],time_cost[1]]]
            for bid in encode_blks_set[pos]:
                if bid!=blockID:
                    mvector_=getminvec(nodeID,bid)
                    if mvector_==[[-1,0]]:
                        return_other_blks.append([[-1,0]])
                    else:
                        return_other_blks.append([[mv[0],mv[2]] for mv in mvector_])
    if GET_TYPE=='bft8-2':
        # 注意，这里的编码方式是不可以改变的，因为数据分片个数参数确定了
        bid=blockID-beginID
        startBlockID=bid-bid%8+beginID
        startEncodeId=startBlockID+ENC_BLK_IDX
        avgsize=encode_shard_pos_bft[startEncodeId][2]
        for i in range(2):
            nid=encode_shard_pos_bft[startEncodeId][i]
            tc=communication_cost_ori[nodeID][nid]*avgsize
            return_encode1.append([nid,tc])
        for i in range(2):
            nid=encode_shard_pos_bft[startEncodeId+1][i]
            tc=communication_cost_ori[nodeID][nid]*avgsize
            return_encode2.append([nid,tc])
        for bid in range(startBlockID,startBlockID+8):
            if bid!=blockID:
                mvector_=getminvec(nodeID,bid)
                if mvector_==[[-1,0]]:
                    return_other_blks.append([[-1,0]])
                    ...
                    return_other_blks.append([[mv[0],mv[2]] for mv in mvector_])
    return [[mv[0],mv[2]] for mv in min_vector],return_encode1,return_encode2,return_other_blks
