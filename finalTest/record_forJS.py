...
def replication_run(max_level,get_average_time=True,get_storage_used=False,get_replica_use_ratio=False):
    ...
    chosen_block_distribution=sys.argv[1]#'nipopows'#'uniform'#'zipf'#'nipopows'#'flyclient'
    # passive alg type
    passive_replicate_type=sys.argv[2]#'popularity'#'popularity'
    # active alg type
    active_replicate_type=sys.argv[3]#'calculate'#'random'#'calculate'
    # how many requests in step epoch
    lambdai=4
    #switch on/off replicate
    passive_on=bool(int(sys.argv[4]))#True
    active_on=bool(int(sys.argv[5]))#True
    # statically assign params
    piece=int(sys.argv[9])#1#3#2#1
    curve_type_replica=sys.argv[10]#'2^n'#'2^n'\'1'\'flyclient'
    # expel params
    period=int(sys.argv[6])#0#3    #np.inf
    curve_type_expel=sys.argv[7]#'1'#'2^n'\'1'\'flyclient'
    expel_type=sys.argv[8]#'curve'#'llu' 'no' 'curve'
    ...
    passive_type='nopassive'
    if passive_on:
        passive_type=passive_replicate_type
    active_type='noactive'
    if active_on:
        active_type=active_replicate_type
    expel_p_type=expel_type
    if expel_type=='curve':
        expel_p_type=curve_type_expel
    ...
    file_average_time='./finalTest/finalRes/A-'+chosen_block_distribution+'-'+curve_type_replica+'-'+str(piece)+'-'+passive_type+str(lambdai)+'-'+active_type+str(top_num_to_offload)+'-'+expel_p_type+'-'+str(period)+'-'+str(last_num_to_expel)+'-'+str(total_times)+'.txt'
    ...
    file_storage_used='./finalTest/finalRes/S-'+chosen_block_distribution+'-'+curve_type_replica+'-'+str(piece)+'-'+passive_type+str(lambdai)+'-'+active_type+str(top_num_to_offload)+'-'+expel_p_type+'-'+str(period)+'-'+str(last_num_to_expel)+'-'+str(total_times)+'.txt'
    ...
    file_use_ratio='./finalTest/finalRes/U-'+chosen_block_distribution+'-'+curve_type_replica+'-'+str(piece)+'-'+passive_type+str(lambdai)+'-'+active_type+str(top_num_to_offload)+'-'+expel_p_type+'-'+str(period)+'-'+str(last_num_to_expel)+'-'+str(total_times)+'.txt'
    ...
    for run_times in range(total_times):
        ...
        for end_since in range(ReplicationAlgorithms.beginID+ReplicationAlgorithms.static_blocks,ReplicationAlgorithms.endID,step):
            ...
            if expel_type!='no':
                ReplicationAlgorithms.expel_blocks(expel_type,end_since,step,last_num_to_expel)
