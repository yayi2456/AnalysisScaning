    ...
    for run_times in range(total_times):
        ...
        for end_since in range(ReplicationAlgorithms_zipf.beginID+ReplicationAlgorithms_zipf.static_blocks,ReplicationAlgorithms_zipf.endID,step):
            ...
            if expel_type!='noexpel':
                ReplicationAlgorithms_zipf.expel_blocks(expel_type,end_since,step,last_num_to_expel)
            # ReplicationAlgorithms_zipf.update_livetime_and_expel(end_since,step)
            ...
            if active_on:
                ...
                for nodeID in nodes_active_replicate:
                    ReplicationAlgorithms_zipf.active_dynamic_replication_one_node(nodeID,top_num_to_offload,active_replicate_type,period)
...
def get_one_total_time_and_replicate(nodeID,end_since,passive_replicate_type,period,block_numbers,chosen_block_distribution,valid_replicate,rank_distribution):
    ...
    for blockID in chosen_blocks:
        ...
        if valid_replicate and min_node!=nodeID:
            ReplicationAlgorithms_zipf.nodes_stored_blocks_popularity[min_node][blockID]+=1
        # record popularity
        chosen_blocks_popularity[blockID]=ReplicationAlgorithms_zipf.nodes_stored_blocks_popularity[min_node][blockID]
        # record time cost
    ...
    if valid_replicate:
        if passive_replicate_type=='popularity':
            ...
            sort_value_dict=[]
        ReplicationAlgorithms_zipf.passive_dynamic_replication_one_node(chosen_blocks,nodeID,passive_replicate_type,sort_value_dict,period)
    # return total time
    return time_used,rd
